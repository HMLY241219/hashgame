...
#[Controller]
class TdslotsController extends AbstractController {
    ...
    public function userAuth(){
        ...
        $userinfo = Db::table('user_token as a')
            ...
            ->selectRaw('br_a.uid,IFNULL(br_b.coin,0) AS coin,IFNULL(br_b.bonus,0) AS bonus')
            ->first();
        //dd($userinfo);
        if (empty($userinfo)){
        ...
        $data = [
            ...
            'currency' => config('tdslots.currency'),
//            'balance' => ($userinfo['coin'] + $userinfo['bonus'])/100,
    ...
    public function userBet(){
        ...
        //单点登录
        $res = $this->slotsCommon->getUserRunGameTage($userinfo['uid'],'td');
    ...
    public function sessionBet(){
        ...
        $table = 'slots_log_' . date('Ymd');
        $over_log = Db::table($table)->where(['is_settlement'=>2,'parentBetId'=>$param['sessionId']])->first();
        if (!empty($over_log)){
            return $this->response->json(['errorCode'=>5, 'message'=>'Other error']);
        }
        ...
        //检查余额是否充足
        if ($old_balance < $param['betAmount'] + $param['preserve']){
            $data['errorCode'] = 2;
            $data['message'] = 'Not enough balance';
            return $this->response->json($data);
        }
    ...
    public static function playerCreated($uid){
        ...
        $url = config('tdslots.api_url').'/CreateMember';
        $data = [
            'Account' => $uid,
            'AgentId' => config('tdslots.AgentId'),
        ];
    ...
    public static function getGameUrl($uid,$gameid){
        ...
        $url = config('tdslots.api_url').'/singleWallet/Login';
        $data = [
            'Token' => $token,
            'GameId' => $gameid,
            'Lang' => config('tdslots.language'),
            'AgentId' => config('tdslots.AgentId'),
        ];
        ...
        $gameUrl = $gameUrl.'&HomeUrl='.config('host.gameurl');
    ...
    public static function KickMember($uid){
        $url = config('tdslots.api_url').'/KickMember';
        $data = [
            'Account' => $uid,
            'AgentId' => config('tdslots.AgentId'),
        ];
    ...
    private static function KeyG(){
        return md5(self::getDateTimeNow().config('tdslots.AgentId').config('tdslots.AgentKey'));
    }
